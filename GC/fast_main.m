% fast_main.m can be used to plotting the results quickly. 
% Since all the data will be load from the .mat files.
% clear all caches
...
save_dir = fullfile(current_dir, 'results_submit');         
if ~exist(save_dir, 'dir')
    error('Results directory not found: %s', save_dir);
end
...
annealing_output = load(fullfile(save_dir, "annealing_output.mat")).annealing_output;
disp(annealing_output);
...
Bounds = load(fullfile(save_dir, "TimeVaryingBoundsData.mat")); 

%% refrence trajectory generation
disp("Generating the reference trajectory.")
generatedTraj = load(fullfile(save_dir, "GeneratedTrajectoryData.mat")); 
...
N = length(t_span); 
initial_file = load(fullfile(save_dir, "InitialPoints.mat")); 
% initial_file = fullfile(save_dir, "InitialPoints_unsatisfy_outsideBox.mat"); 
% initial_file = fullfile(save_dir, "InitialPoints_unsatisfy_outsideBouds.mat"); 
initial = initial_file.initial;
init_n = initial.init_n;
case_name = 'satisfy';
...
for j = 1:init_n
    ...
    run_time(j) = toc;
    fprintf("Tracking Trajectories: %d/%d, runtime = %.4f s, total elapsed time = %.4f s.\n", j, init_n, run_time(j), sum(run_time(1:j)));
